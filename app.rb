...
class App
  ...
  def run
    ...
    puts 'Thanks for using the Student Library Application!'
  end
  ...
  def all_books
    ...
    puts
    puts 'Books'
    puts 'Select option (4) to add a book ' if @books.empty?
    @books.each do |book|
  ...
  def all_people
    ...
    puts
    puts 'People'
    puts 'Select option (3) to add a person ' if @people.empty?
  ...
  def create_student
    ...
    while age <= 0
      age = gets.chomp.to_i
      print 'Add correct age of student: ' if age <= 0
    end
    print 'Name: '
    name = gets.chomp.strip.capitalize
